...
If ((Get-ChildItem *.pdb -Path .\lib -Recurse).Count -gt 0) {
	$packageTask = Create-Process .\NuGet.exe ("pack IkeCode.Core.nuspec -Symbol -Verbosity Detailed")
...
Else {
	$packageTask = Create-Process .\NuGet.exe ("pack IkeCode.Core.nuspec -Verbosity Detailed")
	$packageTask.Start() | Out-Null
	$packageTask.WaitForExit()
