    ...
    public class UrlAttribute : DataTypeAttribute
    {
        ...
        public override string FormatErrorMessage(string name)
        {
            if (ErrorMessage == null && ErrorMessageResourceName == null)
            {
                ...
                {
                    case UrlOptions.RequireProtocol:
                        ErrorMessage = "O campo {0} não é uma URL http, https, ou ftp completa válida.";
                        break;
                    case UrlOptions.OptionalProtocol:
                        ErrorMessage = "O campo {0} não é uma URL válida.";
                        break;
                    case UrlOptions.DisallowProtocol:
                        ErrorMessage = "O campo {0} não é uma URL, sem protocolo, válida.";
                        break;
                    default:
